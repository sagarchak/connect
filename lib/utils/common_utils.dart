...
import 'package:connect/database/ObjectBox.dart';
...
class VersionCode {
  String code = 'Powered by Frontiza';

  VersionCode(this.code);
}

class LocalTimeZone {
  String timezone = 'Kolkata/India';
...
class CommonUtils {
        ...
        child: CommonUtils.checkIfNotNull(imageURL)
            ? CachedNetworkImage(
                ...
                errorWidget: (context, url, error) {
                  ...
                  return Center(
                    child: Image.asset('assets/images/logo.png', height: imageSize),
                  );
                },
              )
            : Container(
                height: imageSize,
                width: imageSize,
                decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular((imageSize / 2)))),
  ...
  static Future<File?> getCompressedImageFromAsset(XFile asset) async {
    if (CommonUtils.checkIfNotNull(asset.path)) {
      // 3. compress asset and get Uint8List.
      final filePath = await FlutterAbsolutePath.getAbsolutePath(asset.path);
      return getCompressedImageFromFilePath(filePath);
    }
    return null;
  }

  static Future<File?> getCompressedImageFromFilePath(String? path) async {
    if (CommonUtils.checkIfNotNull(path)) {
      final Directory extDir = await getApplicationDocumentsDirectory();
      final String dirPath = '${extDir.path}/connect/images';
      await Directory(dirPath).create(recursive: true);
      final String targetFilePath = '$dirPath/${timestamp()}.png';
      File? mFile = await FlutterImageCompress.compressAndGetFile(
          path!,
          targetFilePath,
  ...
  static Future<List<File>> getCompressedFiles({List<String>? imagePathList, List<XFile>? imageAssetList}) async {
    List<File> mFiles = <File>[];
    if (imageAssetList != null && imageAssetList.isNotEmpty) {
      for (XFile asset in imageAssetList) {
        if (CommonUtils.checkIfNotNull(asset.path)) {
          File? mFile = await getCompressedImageFromAsset(asset);
          if (mFile != null && await mFile.exists()) {
            mFiles.add(mFile);
          }
        }
      }
      ...
      for (String path in imagePathList) {
        if (CommonUtils.checkIfNotNull(path)) {
          ...
          if (await mFile.exists()) {
            File? compFile = await getCompressedImageFromFilePath(mFile.path);
            if (compFile != null && await compFile.exists()) {
              mFiles.add(mFile);
            }
          }
        }
  ...
  static Map<String, String> getBodyMap(Map<String, dynamic>? body) {
    Map<String, String> bodyString = <String, String>{};
    if (body != null) {
      for (String key in body.keys) {
        if (CommonUtils.checkIfNotNull(key) && body[key] != null && body[key] is String) {
          bodyString[key] = body[key].toString();
        }
      }
    }
    return bodyString;
  ...
  static Future<List<http.MultipartFile>> getMultipartFiles(String? key, {List<String>? imagePathList, List<XFile>? imageAssetList}) async {
    List<http.MultipartFile> mFiles = <http.MultipartFile>[];
    if (imageAssetList != null && imageAssetList.isNotEmpty) {
      for (XFile asset in imageAssetList) {
        if (CommonUtils.checkIfNotNull(asset.path)) {
          File? mFile = await getCompressedImageFromAsset(asset);
          if (mFile != null && await mFile.exists()) {
            mFiles.add(http.MultipartFile(
              key!,
              mFile.readAsBytes().asStream(),
              mFile.lengthSync(),
  ...
  * Regex for Number starts with 0 r"^(?:[+0])?[0-9]{9,12}$"*/
  ...
  static launchURL(Uri? uri) async {
    if (uri != null && CommonUtils.checkIfNotNull(uri.toString())) {
      ...
      Get.defaultDialog(
        ...
        titleStyle: const TextStyle(fontFamily: FONT_REGULAR, fontSize: 16, color: Colors.white),
        middleTextStyle: const TextStyle(fontFamily: FONT_REGULAR, fontSize: 15, color: Colors.white),
  ...
  static downloadFile(String? resumeLink, String windowTitle) async {
    if (CommonUtils.checkIfNotNull(resumeLink)) {
      ...
      await launchUrl(resumeUrl, mode: LaunchMode.externalApplication, webOnlyWindowName: windowTitle);
    }
  }
  ...
  static Widget getLoaderWidget() {
    return Container(
      ...
      child: SpinKitFadingCube(
        color: HexColor.getColor('04c9e1'),
        size: 100,
      ),
    );
  }
  ...
  static showCommonDialog(String? title, String? message) {
    if (CommonUtils.checkIfNotNull(title) && CommonUtils.checkIfNotNull(message)) {
      Get.defaultDialog(
        title: title ?? 'Info',
        middleText: message ?? "",
        textCancel: "OK",
  ...
  static showCommonFunDialog(String? title, String? message,
      {String textOk = "Ok", String textCancel = "Cancel", Function()? onConfirm, Function()? onCancel}) {
    if (CommonUtils.checkIfNotNull(title) && CommonUtils.checkIfNotNull(message)) {
      Get.defaultDialog(
        title: title ?? 'Info',
        middleText: message ?? "",
  ...
  static showCommonToast(String? title, String? message, {SnackPosition? position, Duration? duration}) {
    if (CommonUtils.checkIfNotNull(title) && CommonUtils.checkIfNotNull(message)) {
  ...
  static showProgressDialog(bool showLoading) {
          ...
          Container(
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                ...
                backgroundColor: HexColor.getColor(PRIMARY_COLOR_LIGHT_HEX),
                minimumSize: Size(150, 45),
                textStyle: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18, fontFamily: 'MontserratMedium'),
  ...
  static printLog(String? log) {
    if (checkIfNotNull(log) && kDebugMode) {
      ...
      for (int i = 0; i <= log!.length / maxLogSize; i++) {
        ...
        debugPrintThrottled(log.substring(start, end));
      }
    }
  }
